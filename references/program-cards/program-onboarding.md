...
draft card generation. The helper writes evidence under
`reports/program-onboarding/<program_key>/<timestamp>/`; see
`evidence-bundle-schema.md`. Install proposal format and execution gates are
defined in `install-proposal-template.md`.
...
submission still require explicit user confirmation. Evidence and draft paths are
project-local by default; `--allow-external-paths` is only for non-install smoke
tests.
...
Use `--default-source github` for tools whose official installation is a GitHub
or source checkout. The selector only records choices; it does not install,
download, scan data, submit jobs, or mutate environments.
...
Default third-version Conda proposal:
