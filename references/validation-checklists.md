...
- First classify the project with `references/project-lifecycle.md` and
  `references/resume-protocol.md` before planning new work.
...
- Assign one primary state: `Project_intake`, `Input_ready`, `Plan_ready`,
  `Script_ready`, `Queued_or_running`, `Failed`, `Complete_unvalidated`,
  `Analysis_ready`, or `Delivered`.
- For `Project_intake`, identify the missing question, scope, design, or explicit
  input needed before planning.
- For `Input_ready`, verify the manifest, sample roles, reference/version, and
  formats before completing the startup planning contract.
- For `Plan_ready`, verify methods, parameters, outputs, resources, risks,
  acceptance gates, and open decisions before generating scripts.
