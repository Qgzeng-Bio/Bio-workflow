...
unchanged until explicitly enabled; use `migration-plan` only and never mutate
legacy paths automatically.

When naming or creating one analysis directory, read
...
If the manifest's declared/inferred analysis type lacks active rule coverage or
evidence, treat checker status `UNCERTAIN` as a claim gate: describe accepted
observations only, label interpretation uncertain, and do not imply that absent
rules equal validation. Read `references/result-interpretation.md` for the
narrative contract.
