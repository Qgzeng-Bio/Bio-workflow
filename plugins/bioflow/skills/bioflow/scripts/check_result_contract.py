#!/usr/bin/env python3
"""Check a result manifest against the bioflow interpretation rules.
...
the calling agent can parse it without a heavy template. Exit code:
  0 = PASS (covered rules passed, no missing fields)
  1 = WARN (WARN/MISSING findings)
  2 = BLOCK (at least one BLOCK rule fired)
  3 = UNCERTAIN (the requested analysis type lacks rule coverage/evidence)
...
def declared_or_inferred_analysis_types(manifest: dict[str, Any]) -> list[str]:
    declared = manifest.get("analysis_types")
    if isinstance(declared, str):
        return [declared]
    if isinstance(declared, list):
        return sorted({str(value).strip() for value in declared if str(value).strip()})

    inferred: list[str] = []
    for analysis_type, keys in SUPPORTED_ANALYSIS_TYPES.items():
        if any(manifest.get(key) for key in keys):
            inferred.append(analysis_type)
    for claim in list_of(manifest, "claims"):
        value = str(claim.get("analysis_type") or "").strip()
        if value and value not in inferred:
            inferred.append(value)
    return sorted(inferred)


def chk_rule_coverage(manifest: dict[str, Any]) -> list[tuple[str, str]]:
    analysis_types = declared_or_inferred_analysis_types(manifest)
    if not analysis_types:
        return [(
            "UNCERTAIN",
            "analysis_types is absent and no supported analysis block can be inferred; "
            "do not make a result claim until scope and rule coverage are explicit",
        )]

    out: list[tuple[str, str]] = []
    for analysis_type in analysis_types:
        keys = SUPPORTED_ANALYSIS_TYPES.get(analysis_type)
        if keys is None:
            out.append((
                "UNCERTAIN",
                f"analysis_type={analysis_type!r} has no active interpretation-rule coverage; "
                "report evidence descriptively and mark interpretation UNCERTAIN",
            ))
        elif not any(manifest.get(key) for key in keys):
            out.append((
                "UNCERTAIN",
                f"analysis_type={analysis_type!r} is declared but none of its evidence blocks "
                f"{list(keys)} are populated",
            ))
    return out
...
def run(manifest: dict, rules: list[dict[str, str]], anchors: dict) -> tuple[str, list[tuple[str, str, str]]]:
    ...
    overall_status is one of PASS / WARN / BLOCK / UNCERTAIN.
    """
    findings: list[tuple[str, str, str]] = []
    for severity, detail in chk_rule_coverage(manifest):
        findings.append((severity, "COVERAGE", detail))
    for rule in rules:
    ...
    if any(s == "UNCERTAIN" for s, _, _ in findings):
...
def render(status: str, findings: list[tuple[str, str, str]]) -> str:
    ...
    if status == "PASS":
        lines.append(
            "\nCOVERED RULES SATISFIED -- claims may proceed only for the supported "
            "analysis types and cited local evidence."
        )
    elif status == "UNCERTAIN":
        lines.append("\nCLAIM COVERAGE INCOMPLETE -- describe observations only and mark interpretation UNCERTAIN.")
    return "\n".join(lines)


def main() -> int:
    parser = argparse.ArgumentParser(description="Check a result manifest against bioflow interpretation rules.")
