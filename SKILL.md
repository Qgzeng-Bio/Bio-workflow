...
- **High-confidence SV (multi-caller consensus):** read `references/playbook-high-confidence-sv-multicaller.md` — orthogonal SV calling with SyRI + SVIM-asm (assembly) + Sniffles2 (reads) on one reference, uniformly normalized and SURVIVOR-merged per sample (`1000 1 0 0 0 50`, union with SUPP_VEC), where read∩assembly cross-support is the high-confidence axis. Call `svcall` env binaries by absolute path (never `micromamba run`); SVIM-asm needs ~96G. (Method draft — run in progress.)
