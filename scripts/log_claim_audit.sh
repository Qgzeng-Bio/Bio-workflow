...
Exit codes (machine-readable):
  ...
  3  UNCERTAIN — analysis type/evidence lacks active rule coverage
  4  INFRA_ERROR — usage/path/yaml/header problem; nothing was checked
  5  PARSE_ERROR — checker output could not be parsed (status/rc mismatch)
USAGE
}
...
uncertain_rules="$(extract_rules "UNCERTAINTY")"
if [[ "$uncertain_rules" != "NONE" ]]; then
    if [[ "$warn_rules" == "NONE" ]]; then
        warn_rules="$uncertain_rules"
    else
        warn_rules="$warn_rules,$uncertain_rules"
    fi
fi
